...
namespace Server.Items
{
	...
	public class Lockpick : Item
	{
        ...
        public void AttemptPick( Mobile from, object targeted )
        {
            if ( Deleted )
                return;
            ...
            {
                if ( Server.Items.DoorType.IsSpaceshipDoor( (BaseDoor)targeted ) && ItemID != 0x3A75 )
                {
                    from.SendMessage( "This doesn't have a key hole, but it does have a card slot." );
                }
                else if ( !(Server.Items.DoorType.IsSpaceshipDoor( (BaseDoor)targeted )) && ItemID == 0x3A75 )
                {
            ...
            else if ( targeted is ILockpickable )
            {
                ...
                if ( Server.Misc.ContainerFunctions.IsSpaceCrate( item.ItemID ) && ((ILockpickable)targeted).Locked && ItemID != 0x3A75 )
                {
                    from.SendMessage( "This doesn't have a key hole, but it does have a card slot." );
                }
                else if ( Server.Misc.ContainerFunctions.IsSpaceCrate( item.ItemID ) && ((ILockpickable)targeted).Locked && ItemID == 0x3A75 )
                {
                    from.PlaySound( 0x54B );
                    new LockpickTimer( from, (ILockpickable)targeted, this ).Start();
                }
                else if ( ((ILockpickable)targeted).Locked && ItemID != 0x3A75 )
                {
                    from.PlaySound( 0x241 );
                    new LockpickTimer( from, (ILockpickable)targeted, this ).Start();
                }
        ...
        private class LockpickTimer : Timer
        {
            ...
            public LockpickTimer( Mobile from, ILockpickable item, Lockpick lockpick ) : base( TimeSpan.FromSeconds( 3.0 ) )
            {
                m_From = from;
                m_Item = item;
                m_Lockpick = lockpick;
		...
		public class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( targeted is ILockpickable )
				{
					...
					{
						from.PlaySound( 0x54B );
						new LockpickTimer( from, (ILockpickable)targeted, m_Item ).Start();
					}
