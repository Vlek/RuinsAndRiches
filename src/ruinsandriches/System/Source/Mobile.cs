...
public class Mobile : IEntity, IHued, IComparable <Mobile>, ISerializable, ISpawnable
{
    ...
    private static char[] m_GhostChars = new char[2] {
        'o', 'O'
    };
    ...
    public void ProcessClothing()
    {
        ...
        if (Utility.ClothingMod())
        {
            ...
            else if (FindItemOnLayer(Layer.OuterTorso) != null && Item.isRobe(FindItemOnLayer(Layer.OuterTorso)))
            {
                ...
                HideItem(FindItemOnLayer(Layer.OuterLegs));
                HideItem(FindItemOnLayer(Layer.Shirt));
                if (FindItemOnLayer(Layer.Pants) != null && !Item.isFullLegs(FindItemOnLayer(Layer.Pants)))
                {
                    HideItem(FindItemOnLayer(Layer.Pants));
                }
                HideItem(FindItemOnLayer(Layer.Waist));
...
//90	only has 0, 1 skeletal knight crowned
    ...
    public int RaceSound(int soundID)
    {
        if (RaceID > 0 || RaceID == -700)
        {
            ...
            else if (soundID == 0x314 || soundID == 0x315 || soundID == 0x316 || soundID == 0x317)
            {
                soundID = RaceDeathSound;
            }                                                                                                                                         // death
            else if (soundID == 0x423 || soundID == 0x424 || soundID == 0x425 || soundID == 0x426 || soundID == 0x427)
            {
                soundID = RaceDeathSound;
            }                                                                                                                                                             // death
            else if (soundID == 0x324 || soundID == 0x325 || soundID == 0x326 || soundID == 0x327 || soundID == 0x328 || soundID == 0x329 || soundID == 0x32A)
            {
                soundID = RaceHurtSound;
            }                                                                                                                                                                                                    // hurt
            else if (soundID == 0x434 || soundID == 0x435 || soundID == 0x436 || soundID == 0x437 || soundID == 0x438 || soundID == 0x439 || soundID == 0x43A || soundID == 0x43B || soundID == 0x43C)
            {
                soundID = RaceHurtSound;
            }                                                                                                                                                                                                                                            // hurt
        }
    ...
    public int SolidHueOverride
    {
        ...
        set { if (m_SolidHueOverride == value)
              {
                  return;
              }
              m_SolidHueOverride = value; Delta(MobileDelta.Hue | MobileDelta.Body); }
    }
