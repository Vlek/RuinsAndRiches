...
public abstract class BaseVendor : BaseCreature, IVendor
{
    ...
    public DateTime NextAction {
        get { return m_NextAction; } set { m_NextAction = value; }
    }

    private const int MaxSell = 500;

    protected abstract List <SBInfo> SBInfos {
        get;
    }
    ...
    public override void OnAfterSpawn()
    {
        ...
        else if (Server.Misc.Worlds.IsSeaTown(this.Location, this.Map))
        {
            ...
            {
                this.Title = "the dock worker"; if (Utility.RandomBool())
                {
                    this.Title = "the merchant";
                }
            }
            ...
            {
                this.Title = "the cabin boy"; if (this.Female)
                {
    ...
    public override bool OnDragDrop(Mobile from, Item dropped)
    {
        ...
        else
        {
            ...
            if (RelicValue > 0)
            {
                ...
                {
                    gBonus = gBonus + (int)Math.Round(((100.00 * RelicValue) / 100), 0); GuildMember = 1;
                }                                                                        // FOR GUILD MEMBERS
            ...
            else if (dropped is Item && !(dropped is Gold) && this is Thief)
            {
                ...
                if (iAmThief < 10)
                {
                    this.PrivateOverheadMessage(MessageType.Regular, 1153, false, "I only deal with fellow thieves.", from.NetState);
                }
                else if (dropped is StealBox || dropped is StealMetalBox || dropped is StealBag)
                {
                    ...
                    if (this.NpcGuild != NpcGuild.None && this.NpcGuild == pm.NpcGuild)
                    {
                        gBonus = gBonus + (int)Math.Round(((100.00 * 500) / 100), 0); GuildMember = 1;
                    }                                                                            // FOR GUILD MEMBERS
                    if (BeggingPose(from) > 0 && GuildMember == 0)                               // LET US SEE IF THEY ARE BEGGING
                    {
                        Titles.AwardKarma(from, -BeggingKarma(from), true);
                ...
                else if (dropped is StolenChest)
                {
                    ...
                    if (this.NpcGuild != NpcGuild.None && this.NpcGuild == pm.NpcGuild)
                    {
                        gBonus = gBonus + (int)Math.Round(((100.00 * vRipoff) / 100), 0); GuildMember = 1;
                    }                                                                            // FOR GUILD MEMBERS
            ...
            else if ((dropped is HenchmanFighterItem || dropped is HenchmanArcherItem || dropped is HenchmanWizardItem) && (this is InnKeeper || this is TavernKeeper || this is Barkeeper))
            {
                ...
                {
                    this.PrivateOverheadMessage(MessageType.Regular, 1153, false, "This is not a graveyard! Bury them somewhere else!", from.NetState);
                }
            }
