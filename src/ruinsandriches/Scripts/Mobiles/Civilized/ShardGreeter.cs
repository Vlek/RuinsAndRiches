...
public class GypsyTarotGump : Gump
{
    ...
    public string cardText(int page, int section, int creature)
    {
        ...
        if (creature > 0)
        {
            ...
            if (Server.Items.BaseRace.GetUndead(creature))
            {
                undead = " Although you do not remember your past life, you feel different from the other undead. You seem to have retained your soul, which will surely be noticed by other undead. This means they will likely attack you as they do the living.";
            }

            if (Server.Items.BaseRace.BloodDrinker(creature))
            {
                undead = undead + " Having a soul, however, means you can safely walk the land during the daylight.";
            }
