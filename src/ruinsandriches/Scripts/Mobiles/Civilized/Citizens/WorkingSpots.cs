...
public class WorkingSpots : Item
{
    ...
    public static void Setup()
    {
        ...
        for (int i = 0; i < spots.Count; ++i)
        {
            ...
            {
                Mobile citizen = new TradesmanCook(); citizen.MoveToWorld(spot.Location, spot.Map); ((BaseCreature)citizen).Home = spot.Location; ((BaseCreature)citizen).RangeHome = 0; citizen.OnAfterSpawn(); Add(citizen, spot, 1);
            }
            else if (spot.Name == "butcher")
            {
                Mobile citizen = new TradesmanButcher(); citizen.MoveToWorld(spot.Location, spot.Map); ((BaseCreature)citizen).Home = spot.Location; ((BaseCreature)citizen).RangeHome = 0; citizen.OnAfterSpawn(); Add(citizen, spot, 1);
            }
            else if (spot.Name == "bard")
            {
                Mobile citizen = new TradesmanBard(); citizen.MoveToWorld(spot.Location, spot.Map); ((BaseCreature)citizen).Home = spot.Location; ((BaseCreature)citizen).RangeHome = 0; citizen.OnAfterSpawn(); Add(citizen, spot, 1);
            }
            else if (spot.Name == "shrine")
            {
                Mobile citizen = new TrainingSpirits(); citizen.MoveToWorld(spot.Location, spot.Map); ((BaseCreature)citizen).Home = spot.Location; ((BaseCreature)citizen).RangeHome = 0; citizen.OnAfterSpawn(); Add(citizen, spot, 6);
            }
        }
    }
    ...
    public static void Add(Mobile citizen, Item spots, int range)
    {
        ...
        for (int i = 0; i < items.Count; ++i)
        {
            if (hasCitizen)
            {
                ...
                {
                    Item item = new StoveHit(); item.MoveToWorld(spot.Location, spot.Map); item.Visible = false; item.Weight = -2.0;
                }
                else if (spot.Name == "pan" && citizen is TradesmanCook)
                {
                    Item item = new StoveHit(); item.MoveToWorld(spot.Location, spot.Map); item.Weight = -2.0;
                }
                else if (spot.Name == "carcass" && citizen is TradesmanButcher)
                {
                    Item item = new ButcherHit(); item.MoveToWorld(spot.Location, spot.Map); item.Weight = -2.0;
                }
                else if (spot.Name == "music" && citizen is TradesmanBard)
                {
                    Item item = new BardHit(); item.MoveToWorld(spot.Location, spot.Map); item.Weight = -2.0;
                }
            }
        }
    }
