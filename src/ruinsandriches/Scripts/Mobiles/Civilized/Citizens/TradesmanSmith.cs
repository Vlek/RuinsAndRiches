    ...
    public override void OnThink()
    {
        if (DateTime.Now >= m_NextTalk)
        {
            foreach (Item anvil in this.GetItemsInRange(1))
            {
                ...
                {
                    if (this.FindItemOnLayer(Layer.FirstValid) != null && !(this.FindItemOnLayer(Layer.FirstValid) is Club))
                    {
                        this.Delete();
                    }
                    else if (this.FindItemOnLayer(Layer.OneHanded) != null && !(this.FindItemOnLayer(Layer.OneHanded) is Club))
                    {
                        this.Delete();
                    }
                    else if (this.FindItemOnLayer(Layer.TwoHanded) != null && !(this.FindItemOnLayer(Layer.TwoHanded) is Club))
                    {
                        this.Delete();
                    }
