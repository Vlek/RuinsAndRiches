    ...
    public override void OnThink()
    {
        if (DateTime.Now >= m_NextTalk)
        {
            foreach (Item carcass in this.GetItemsInRange(1))
            {
                ...
                {
                    if (this.FindItemOnLayer(Layer.FirstValid) != null && !(this.FindItemOnLayer(Layer.FirstValid) is Cleaver))
                    {
                        this.Delete();
                    }
                    else if (this.FindItemOnLayer(Layer.OneHanded) != null && !(this.FindItemOnLayer(Layer.OneHanded) is Cleaver))
                    {
                        this.Delete();
                    }
