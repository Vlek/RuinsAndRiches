    ...
    public override void OnThink()
    {
        if (DateTime.Now >= m_NextTalk)
        {
            foreach (Item hay in this.GetItemsInRange(6))
            {
                ...
                {
                    if (this.FindItemOnLayer(Layer.FirstValid) != null && !(this.FindItemOnLayer(Layer.FirstValid) is BaseRanged))
                    {
                        this.Delete();
                    }
                    else if (this.FindItemOnLayer(Layer.TwoHanded) != null && !(this.FindItemOnLayer(Layer.TwoHanded) is BaseRanged))
                    {
                        this.Delete();
                    }
