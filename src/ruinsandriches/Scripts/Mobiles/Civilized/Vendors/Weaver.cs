...
public class Weaver : BaseVendor
{
    ...
    public void BeginRepair(Mobile from)
    {
        ...
        if (BeggingPose(from) > 0)                   // LET US SEE IF THEY ARE BEGGING
        {
            ...
            nCostH = nCostH - (int)((from.Skills[SkillName.Begging].Value * 0.005) * nCostH); if (nCostH < 1)
            {
                nCostH = 1;
            }
            idCost = idCost - (int)((from.Skills[SkillName.Begging].Value * 0.005) * idCost); if (idCost < 1)
            {
                idCost = 1;
            }
        ...
        {
            SayTo(from, "If you want me to tailor your robe or cloak to look normal, it will cost you " + nCost.ToString() + " gold. Maybe repair a hat at " + nCostH.ToString() + " gold per durability? Or maybe identify an item for " + idCost.ToString() + " gold?");
        }
