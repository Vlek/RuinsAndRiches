...
public class Herbalist : BaseVendor
{
    ...
    public void BeginRepair(Mobile from)
    {
        ...
        {
            SayTo(from, "If you want me to identify unknown reagents, it will cost you " + nCost.ToString() + " gold.");
        }
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            ...
            if (targeted is UnknownReagent && from.Backpack != null)
            {
                ...
                if (pack.ConsumeTotal(typeof(Gold), toConsume))
                {
                    ...
                    if (reagentType == 0)
                    {
                        from.AddToBackpack(new BlackPearl(RegCount)); reagentName = "black pearl";
                    }
                    else if (reagentType == 1)
                    {
                        from.AddToBackpack(new Bloodmoss(RegCount)); reagentName = "bloodmoss";
                    }
                    else if (reagentType == 2)
                    {
                        from.AddToBackpack(new Garlic(RegCount)); reagentName = "garlic";
                    }
                    else if (reagentType == 3)
                    {
                        from.AddToBackpack(new Ginseng(RegCount)); reagentName = "ginseng";
                    }
                    else if (reagentType == 4)
                    {
                        from.AddToBackpack(new MandrakeRoot(RegCount)); reagentName = "mandrake root";
                    }
                    else if (reagentType == 5)
                    {
                        from.AddToBackpack(new Nightshade(RegCount)); reagentName = "nightshade";
                    }
                    else if (reagentType == 6)
                    {
                        from.AddToBackpack(new SulfurousAsh(RegCount)); reagentName = "sulfurous ash";
                    }
                    else if (reagentType == 7)
                    {
                        from.AddToBackpack(new SpidersSilk(RegCount)); reagentName = "spiders silk";
                    }
                    else if (reagentType == 8)
                    {
                        from.AddToBackpack(new BatWing(RegCount)); reagentName = "bat wing";
                    }
                    else if (reagentType == 9)
                    {
                        from.AddToBackpack(new GraveDust(RegCount)); reagentName = "grave dust";
                    }
                    else if (reagentType == 10)
                    {
                        from.AddToBackpack(new DaemonBlood(RegCount)); reagentName = "daemon blood";
                    }
                    else if (reagentType == 11)
                    {
                        from.AddToBackpack(new NoxCrystal(RegCount)); reagentName = "nox crystal";
                    }
                    else if (reagentType == 12)
                    {
                        from.AddToBackpack(new PigIron(RegCount)); reagentName = "pig iron";
                    }
                    else if (reagentType == 13)
                    {
                        from.AddToBackpack(new BitterRoot(RegCount)); reagentName = "bitter root";
                    }
                    else if (reagentType == 14)
                    {
                        from.AddToBackpack(new BlackSand(RegCount)); reagentName = "black sand";
                    }
                    else if (reagentType == 15)
                    {
                        from.AddToBackpack(new BloodRose(RegCount)); reagentName = "blood rose";
                    }
                    else if (reagentType == 16)
                    {
                        from.AddToBackpack(new DriedToad(RegCount)); reagentName = "dried toad";
                    }
                    else if (reagentType == 17)
                    {
                        from.AddToBackpack(new Maggot(RegCount)); reagentName = "maggot";
                    }
                    else if (reagentType == 18)
                    {
                        from.AddToBackpack(new MummyWrap(RegCount)); reagentName = "mummy wrap";
                    }
                    else if (reagentType == 19)
                    {
                        from.AddToBackpack(new VioletFungus(RegCount)); reagentName = "violet fungus";
                    }
                    else if (reagentType == 20)
                    {
                        from.AddToBackpack(new WerewolfClaw(RegCount)); reagentName = "werewolf claw";
                    }
                    else if (reagentType == 21)
                    {
                        from.AddToBackpack(new Wolfsbane(RegCount)); reagentName = "wolfsbane";
                    }
                    else if (reagentType == 22)
                    {
                        from.AddToBackpack(new SackFlour()); RegCount = 1; reagentName = "regular flour";
                    }
                    else if (reagentType == 23)
                    {
                        from.AddToBackpack(new FertileDirt()); RegCount = 1; reagentName = "plain dirt";
                    }
                    else
                    {
                        from.AddToBackpack(new Sand()); RegCount = 1; reagentName = "some sand";
                    }
                    ...
                    {
                        m_Herbalist.SayTo(from, "This seems to be " + RegCount + " " + reagentName + ".");
                    }
                    weed.Delete();
                }
                else
