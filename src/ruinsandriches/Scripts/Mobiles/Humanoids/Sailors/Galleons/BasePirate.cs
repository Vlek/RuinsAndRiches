...
public class BasePirate : BaseCreature
{
    ...
    {
        int title = 1;
        if (Server.Misc.Worlds.GetMyWorld(captain.Map, captain.Location, captain.X, captain.Y) == "the Land of Lodoria")
        {
            title = Utility.RandomMinMax(1, 10);
        }
        else if (Server.Misc.Worlds.GetMyWorld(captain.Map, captain.Location, captain.X, captain.Y) == "the Land of Sosaria")
        {
            title = Utility.RandomMinMax(11, 18);
        }
        else if (Server.Misc.Worlds.GetMyWorld(captain.Map, captain.Location, captain.X, captain.Y) == "the Savaged Empire")
        {
            title = Utility.RandomMinMax(22, 23);
        }
        else if (Server.Misc.Worlds.GetMyWorld(captain.Map, captain.Location, captain.X, captain.Y) == "the Isles of Dread")
        {
            title = 21;
        }
        else if (Server.Misc.Worlds.GetMyWorld(captain.Map, captain.Location, captain.X, captain.Y) == "the Island of Umber Veil")
        {
            title = 19;
        }
        else if (Server.Misc.Worlds.GetMyWorld(captain.Map, captain.Location, captain.X, captain.Y) == "the Bottle World of Kuldar")
        {
            title = 20;
        }
