...
public class ShinobiScroll : Item
{
    ...
    public int Page_ {
        get { return Page; } set { Page = value; InvalidateProperties(); }
    }

    [CommandProperty(AccessLevel.Owner)]
    public int CheetahPaws_ {
        get { return CheetahPaws; } set { CheetahPaws = value; InvalidateProperties(); }
    }
    [CommandProperty(AccessLevel.Owner)]
    public int Deception_ {
        get { return Deception; } set { Deception = value; InvalidateProperties(); }
    }
    [CommandProperty(AccessLevel.Owner)]
    public int EagleEye_ {
        get { return EagleEye; } set { EagleEye = value; InvalidateProperties(); }
    }
    [CommandProperty(AccessLevel.Owner)]
    public int Espionage_ {
        get { return Espionage; } set { Espionage = value; InvalidateProperties(); }
    }
    [CommandProperty(AccessLevel.Owner)]
    public int FerretFlee_ {
        get { return FerretFlee; } set { FerretFlee = value; InvalidateProperties(); }
    }
    [CommandProperty(AccessLevel.Owner)]
    public int MonkeyLeap_ {
        get { return MonkeyLeap; } set { MonkeyLeap = value; InvalidateProperties(); }
    }
    [CommandProperty(AccessLevel.Owner)]
    ...
    public int TigerStrength_ {
        get { return TigerStrength; } set { TigerStrength = value; InvalidateProperties(); }
    }
    ...
    public override bool OnDragDrop(Mobile from, Item dropped)
    {
        ...
        {
            if (CheetahPaws < 1 && (dropped.Name == ShinobiInfo(290, "prize1") || dropped.Name == ShinobiInfo(290, "prize2")))
            {
                win = ShinobiInfo(290, "name"); CheetahPaws = 1;
            }
            else if (Deception < 1 && (dropped.Name == ShinobiInfo(291, "prize1") || dropped.Name == ShinobiInfo(291, "prize2")))
            {
                win = ShinobiInfo(291, "name"); Deception = 1;
            }
            else if (EagleEye < 1 && (dropped.Name == ShinobiInfo(292, "prize1") || dropped.Name == ShinobiInfo(292, "prize2")))
            {
                win = ShinobiInfo(292, "name"); EagleEye = 1;
            }
            else if (Espionage < 1 && (dropped.Name == ShinobiInfo(293, "prize1") || dropped.Name == ShinobiInfo(293, "prize2")))
            {
                win = ShinobiInfo(293, "name"); Espionage = 1;
            }
            else if (FerretFlee < 1 && (dropped.Name == ShinobiInfo(294, "prize1") || dropped.Name == ShinobiInfo(294, "prize2")))
            {
                win = ShinobiInfo(294, "name"); FerretFlee = 1;
            }
            else if (MonkeyLeap < 1 && (dropped.Name == ShinobiInfo(295, "prize1") || dropped.Name == ShinobiInfo(295, "prize2")))
            {
                win = ShinobiInfo(295, "name"); MonkeyLeap = 1;
            }
            else if (MysticShuriken < 1 && (dropped.Name == ShinobiInfo(296, "prize1") || dropped.Name == ShinobiInfo(296, "prize2")))
            {
                win = ShinobiInfo(296, "name"); MysticShuriken = 1;
            }
            else if (TigerStrength < 1 && (dropped.Name == ShinobiInfo(297, "prize1") || dropped.Name == ShinobiInfo(297, "prize2")))
            {
                win = ShinobiInfo(297, "name"); TigerStrength = 1;
            }
    ...
    public class ShinobiScrollGump : Gump
    {
        ...
        public ShinobiScrollGump(Mobile from, ShinobiScroll scroll) : base(100, 100)
        {
            ...
            int left = scroll.Page - 1;     if (left < 1)
            {
                left = 10;
            }
            int right = scroll.Page + 1;    if (right > 10)
            {
                right = 1;
            }
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 21)
            {
                from.SendGump(new ShinobiRow(from, mScroll));               showScroll = false;
            }
            else if (info.ButtonID == 22)
            {
                from.SendGump(new ShinobiColumn(from, mScroll));    showScroll = false;
            }
    ...
    public class ShinobiRow : Gump
    {
        ...
        public ShinobiRow(Mobile from, ShinobiScroll scroll) : base(25, 25)
        {
            ...
            {
                line = line + 50; AddButton(line, 2, 10876, 10876, 290, GumpButtonType.Reply, 0);
            }
            if (scroll.Deception > 0)
            {
                line = line + 50; AddButton(line, 2, 10871, 10871, 291, GumpButtonType.Reply, 0);
            }
            if (scroll.EagleEye > 0)
            {
                line = line + 50; AddButton(line, 2, 10872, 10872, 292, GumpButtonType.Reply, 0);
            }
            if (scroll.Espionage > 0)
            {
                line = line + 50; AddButton(line, 2, 10873, 10873, 293, GumpButtonType.Reply, 0);
            }
            if (scroll.FerretFlee > 0)
            {
                line = line + 50; AddButton(line, 2, 10874, 10874, 294, GumpButtonType.Reply, 0);
            }
            if (scroll.MonkeyLeap > 0)
            {
                line = line + 50; AddButton(line, 2, 10875, 10875, 295, GumpButtonType.Reply, 0);
            }
            if (scroll.MysticShuriken > 0)
            {
                line = line + 50; AddButton(line, 2, 10877, 10877, 296, GumpButtonType.Reply, 0);
            }
            if (scroll.TigerStrength > 0)
            {
                line = line + 50; AddButton(line, 2, 10878, 10878, 297, GumpButtonType.Reply, 0);
            }
        }
    ...
    public class ShinobiColumn : Gump
    {
        ...
        public ShinobiColumn(Mobile from, ShinobiScroll scroll) : base(25, 25)
        {
            ...
            if (scroll.CheetahPaws > 0)
            {
                line = line + 50; AddButton(2, line, 10876, 10876, 290, GumpButtonType.Reply, 0);
            }
            if (scroll.Deception > 0)
            {
                line = line + 50; AddButton(2, line, 10871, 10871, 291, GumpButtonType.Reply, 0);
            }
            if (scroll.EagleEye > 0)
            {
                line = line + 50; AddButton(2, line, 10872, 10872, 292, GumpButtonType.Reply, 0);
            }
            if (scroll.Espionage > 0)
            {
                line = line + 50; AddButton(2, line, 10873, 10873, 293, GumpButtonType.Reply, 0);
            }
            if (scroll.FerretFlee > 0)
            {
                line = line + 50; AddButton(2, line, 10874, 10874, 294, GumpButtonType.Reply, 0);
            }
            if (scroll.MonkeyLeap > 0)
            {
                line = line + 50; AddButton(2, line, 10875, 10875, 295, GumpButtonType.Reply, 0);
            }
            if (scroll.MysticShuriken > 0)
            {
                line = line + 50; AddButton(2, line, 10877, 10877, 296, GumpButtonType.Reply, 0);
            }
            if (scroll.TigerStrength > 0)
            {
                line = line + 50; AddButton(2, line, 10878, 10878, 297, GumpButtonType.Reply, 0);
            }
        }
