...
public class ResearchDeathSpeak : ResearchSpell
{
    ...
    public override void OnCast()
    {
        ...
        if (regenerate > 0)
        {
            ...
            if (mana > regenerate)
            {
                Caster.Mana = Caster.Mana + regenerate; regenerate = 0;
            }
            else
            {
                Caster.Mana = Caster.ManaMax; regenerate = regenerate - mana;
            }

            int stam = Caster.ManaMax - Caster.Stam;
            if (stam > regenerate)
            {
                Caster.Stam = Caster.Stam + regenerate; regenerate = 0;
            }
            else
            {
                Caster.Stam = Caster.StamMax; regenerate = regenerate - stam;
            }

            int hits = Caster.ManaMax - Caster.Hits;
            if (hits > regenerate)
            {
                Caster.Hits = Caster.Hits + regenerate; regenerate = 0;
            }
            else
            {
                Caster.Hits = Caster.HitsMax; regenerate = regenerate - hits;
            }
