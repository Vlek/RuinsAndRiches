        ...
        else if (SpellHelper.CheckTown(p, Caster) && CheckSequence())
        {
            ...
            if (targets.Count > 0)
            {
                ...
                for (int i = 0; i < targets.Count; ++i)
                {
                    ...
                    {
                        Effects.SendLocationEffect(blast2, m.Map, 0x23B2, 85, 10, Server.Misc.PlayerSettings.GetMySpellHue(true, Caster, 0), 0);
                    }
                    if (Utility.RandomBool())
                    {
                        Effects.SendLocationEffect(blast3, m.Map, 0x23B2, 85, 10, Server.Misc.PlayerSettings.GetMySpellHue(true, Caster, 0), 0);
                    }
                    if (Utility.RandomBool())
                    {
                        Effects.SendLocationEffect(blast4, m.Map, 0x23B2, 85, 10, Server.Misc.PlayerSettings.GetMySpellHue(true, Caster, 0), 0);
                    }
                    if (Utility.RandomBool())
                    {
                        Effects.SendLocationEffect(blast5, m.Map, 0x23B2, 85, 10, Server.Misc.PlayerSettings.GetMySpellHue(true, Caster, 0), 0);
                    }
