        ...
        if (CheckSequence())
        {
            ...
            switch (Utility.RandomMinMax(1, 28))
            {
                case 1: item = new Apple(); item.Amount = Utility.RandomMinMax(1, 5); msg = "You conjure some apples.";
                    ...
                    {
                        item = new BloodyDrink(); item.Amount = Utility.RandomMinMax(1, 5); msg = "You conjure fresh blood.";
                    }
                    else if (Server.Items.BaseRace.BrainEater(Caster.RaceID))
                    {
                        item = new FreshBrain(); item.Amount = Utility.RandomMinMax(1, 5); msg = "You conjure fresh brains.";
                    }
                    break;
