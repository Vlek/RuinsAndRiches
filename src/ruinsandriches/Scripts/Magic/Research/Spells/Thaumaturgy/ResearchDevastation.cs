...
public class ResearchDevastation : ResearchSpell
{
    ...
    public override double RequiredSkill {
        get { return (double)(Int32.Parse(Server.Misc.Research.SpellInformation(spellIndex, 8))); }
    }
        ...
        else if (SpellHelper.CheckTown(p, Caster) && CheckSequence())
        {
            ...
            if (targets.Count > 0)
            {
                for (int i = 0; i < targets.Count; ++i)
                {
                    ...
                    if (Utility.RandomBool())
                    {
                        Effects.SendLocationEffect(blast2, m.Map, Utility.RandomList(0x36BD, 0x3822), 85, 10, Server.Misc.PlayerSettings.GetMySpellHue(true, Caster, 0), 0);
                    }
                    if (Utility.RandomBool())
                    {
                        Effects.SendLocationEffect(blast3, m.Map, Utility.RandomList(0x36BD, 0x3822), 85, 10, Server.Misc.PlayerSettings.GetMySpellHue(true, Caster, 0), 0);
                    }
                    if (Utility.RandomBool())
                    {
                        Effects.SendLocationEffect(blast4, m.Map, Utility.RandomList(0x36BD, 0x3822), 85, 10, Server.Misc.PlayerSettings.GetMySpellHue(true, Caster, 0), 0);
                    }
                    if (Utility.RandomBool())
                    {
                        Effects.SendLocationEffect(blast5, m.Map, Utility.RandomList(0x36BD, 0x3822), 85, 10, Server.Misc.PlayerSettings.GetMySpellHue(true, Caster, 0), 0);
                    }
                    ...
                    {
                        m.Damage(10000, Caster); Caster.Criminal = true;
                    }
