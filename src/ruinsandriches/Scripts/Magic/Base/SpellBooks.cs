...
class BookProperties
{
    public static void GetBookProperties(Spellbook book, int nCycle, int nChance)
    {
        ...
        if (chance >= Utility.Random(100))
        {
            ...
            {
                book.SkillBonuses.SetValues(2, m_MiscDeadSkills[Utility.Random(m_MiscDeadSkills.Length)], Utility.RandomMinMax(1, 10));
            }
            else
            {
                book.SkillBonuses.SetValues(2, m_MiscSkills[Utility.Random(m_MiscSkills.Length)], Utility.RandomMinMax(1, 10));
            }
        }
    }
