...
public class StealBase : BaseAddon
{
    ...
    public int Box_Type {
        get { return BoxColor; } set { BoxColor = value; InvalidateProperties(); }
    }

    [CommandProperty(AccessLevel.Owner)]
    public int Box_Color {
        get { return BoxType; } set { BoxType = value; InvalidateProperties(); }
    }

    [CommandProperty(AccessLevel.Owner)]
    public int Ped_Type {
        get { return PedType; } set { PedType = value; InvalidateProperties(); }
    }

    [CommandProperty(AccessLevel.Owner)]
    public string Box_Origin {
        get { return BoxOrigin; } set { BoxOrigin = value; InvalidateProperties(); }
    }

    [CommandProperty(AccessLevel.Owner)]
    public string Box_Carving {
        get { return BoxCarving; } set { BoxCarving = value; InvalidateProperties(); }
    }

    public int m_Tries;
    public int Tries {
        get { return m_Tries; } set { m_Tries = value; }
    }
    ...
    public void FillMeUp(Container box, Mobile from)
    {
        ...
        if (Server.Misc.GetPlayerInfo.LuckyPlayer((int)(20 + (from.Luck / 2))) == true)
        {
            ...
            else
            {
                ...
                if (relic is DDRelicWeapon && Server.Misc.GetPlayerInfo.OrientalPlay(from) == true)
                {
                    Server.Items.DDRelicWeapon.MakeOriental(relic);
                }
                else if (relic is DDRelicStatue && Server.Misc.GetPlayerInfo.OrientalPlay(from) == true)
                {
                    Server.Items.DDRelicStatue.MakeOriental(relic);
                }
                else if (relic is DDRelicBanner && relic.ItemID != 0x2886 && relic.ItemID != 0x2887 && Server.Misc.GetPlayerInfo.OrientalPlay(from) == true)
                {
        ...
        if (Server.Misc.GetPlayerInfo.LuckyPlayer((int)(20 + (from.Luck / 2))) == true)
        {
            ...
            if (scroll > 2)
            {
                i = Loot.RandomScroll(47, 63, SpellbookType.Regular); box.DropItem(i);
            }
            else if (scroll == 2)
            {
                i = Loot.RandomScroll(20, 31, SpellbookType.Elementalism); box.DropItem(i);
            }
            else
            {
                i = Loot.RandomScroll(0, 17, SpellbookType.Necromancer); box.DropItem(i);
            }
        }
