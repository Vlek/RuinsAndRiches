...
public class QuickBar : Gump
{
    ...
    public override void OnResponse(NetState sender, RelayInfo info)
    {
        ...
        if (info.ButtonID > 0)
        {
            from.SendGump(new QuickBar(from)); from.SendSound(0x4A);
        }
        ...
        {
            from.CloseGump(typeof(QuestsGump)); from.SendGump(new QuestsGump(from));
        }
        ...
        {
            from.CloseGump(typeof(MyLibrary)); from.SendGump(new MyLibrary(from, 0)); from.SendSound(0x4A);
        }
        else if (info.ButtonID == 24)
        {
            from.CloseGump(typeof(MyChat)); from.SendGump(new MyChat(from, 0)); from.SendSound(0x4A);
        }
        ...
        else if (info.ButtonID == 31)
        {
            ...
            {
                from.CloseGump(typeof(BagOfTricks.TricksLargeColumn));      from.SendGump(new BagOfTricks.TricksLargeRow(from));
            }
            else if (from.HasGump(typeof(BagOfTricks.TricksLargeRow)))
            {
                from.CloseGump(typeof(BagOfTricks.TricksLargeRow));         from.SendGump(new BagOfTricks.TricksSmallColumn(from));
            }
            else if (from.HasGump(typeof(BagOfTricks.TricksSmallColumn)))
            {
                from.CloseGump(typeof(BagOfTricks.TricksSmallColumn));      from.SendGump(new BagOfTricks.TricksSmallRow(from));
            }
