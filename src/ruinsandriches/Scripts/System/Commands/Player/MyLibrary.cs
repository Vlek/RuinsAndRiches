    ...
    public override void OnResponse(NetState sender, RelayInfo info)
    {
        ...
        if (button > 0)
        {
            ...
            if (button >= 400)                       // BUILT IN HELP
            {
                if (button == 400)
                {
                    from.CloseGump(typeof(BasicsGump)); from.SendGump(new BasicsGump(from, 0));
                }
                else if (button == 401)
                {
                    from.CloseGump(typeof(CreatureHelpGump)); from.SendGump(new CreatureHelpGump(from, 0));
                }
                else if (button == 402)
                {
                    from.CloseGump(typeof(FameKarma)); from.SendGump(new FameKarma(from, 0));
                }
                else if (button == 403)
                {
                    from.CloseGump(typeof(ItemPropsGump)); from.SendGump(new ItemPropsGump(from, 0));
                }
                else if (button == 404)
                {
                    from.CloseGump(typeof(NewSkillsGump)); from.SendGump(new NewSkillsGump(from, 0));
                }
                else
                {
                    from.CloseGump(typeof(WeaponAbilityBook.AbilityBookGump)); from.SendGump(new WeaponAbilityBook.AbilityBookGump(from));
                }
            }
            ...
            else                     // LORE BOOKS
            {
                ...
                if (item is LoreBook)
                {
                    LoreBook lore = (LoreBook)item; lore.writeBook(refer);
                }
                item.OnDoubleClick(from);
                item.Delete();
            }
