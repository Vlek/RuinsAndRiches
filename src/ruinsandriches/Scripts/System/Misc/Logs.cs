...
class LoggingFunctions
{
    ...
    public static string LogRead(string sLog, Mobile m)
    {
            ...
            using (reader = new StreamReader(sPath))
            {
                while (!reader.EndOfStream)
                {
                    ...
                    foreach (string eachWords in eachWord)
                    {
                        if (nLine == 1)
                        {
                            nLine = 2; sLogEntries = sLogEntries + eachWords + ".<br>" + sBreak; nBlank = 0;
                        }
                        else
                        {
                            nLine = 1; sLogEntries = sLogEntries + " - " + eachWords + "<br><br>";
                        }
                    }
                }
            }
        ...
        if (nBlank == 1)
        {
            if (sLog == "Logging Murderers")
            {
                sLogEntries = sLogEntries + "I am happy to say " + m.Name + ", that no one is wanted for murder.";
            }
            else if (sLog == "Logging Battles")
            {
                sLogEntries = sLogEntries + "Sorry, " + m.Name + ". I have no new tales of bravery to tell.";
            }
            else if (sLog == "Logging Adventures")
            {
                sLogEntries = sLogEntries + "Sorry, " + m.Name + ". I have no new gossip to tell.";
            }
            else if (sLog == "Logging Quests")
            {
                sLogEntries = sLogEntries + "Sorry, " + m.Name + ". I have no new tales of deeds to tell.";
            }
            else if (sLog == "Logging Deaths")
            {
                sLogEntries = sLogEntries + "I am happy to say " + m.Name + ", that all of Sosaria's citizens are alive and well.";
            }
            else if (sLog == "Logging Journies")
            {
                sLogEntries = sLogEntries + "Sorry, " + m.Name + ". I have no new tales of exploration to tell.";
            }
            else
            {
                sLogEntries = sLogEntries + "Sorry, " + m.Name + ". I have nothing new to tell of such things.";
            }
        }
    ...
    {
        int i = 0;
        using (StreamReader r = new StreamReader(filePath)){ while (r.ReadLine() != null)
                                                             {
                                                                 i++;
                                                             }
        }
    ...
    public static string LogSpeak()
    {
        ...
        {
            mySpeaking = mySpeaking.Replace(" made a fatal mistake ", " making a fatal mistake ");
        }
    ...
    public static string LogRegions(Mobile m, string sRegion, string sDirection)
    {
        ...
        if ((m is PlayerMobile) && (m.AccessLevel < AccessLevel.GameMaster))
        {
            ...
            if (pm.PublicMyRunUO == true)
            {
                ...
                if (sDirection == "enter")
                {
                    sEvent = m.Name + " " + sTitle + " entered " + sRegion + "#" + sDateString; LoggingFunctions.LogEvent(sEvent, "Logging Journies");
                }
    ...
    public static string LogKillers(Mobile m, int nKills)
    {
        ...
        if (m.Kills > 1)
        {
            sEvent = m.Name + " " + sTitle + " is wanted for the murder of " + m.Kills + " people.";
        }
        else if (m.Kills > 0)
        {
