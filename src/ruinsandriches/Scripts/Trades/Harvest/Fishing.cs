...
public class Fishing : HarvestSystem
{
    ...
    public static double PoleSkill(Mobile m)
    {
        ...
        if (m.FindItemOnLayer(Layer.OneHanded) != null && m.FindItemOnLayer(Layer.OneHanded) is FishingPole)
        {
            pole = m.FindItemOnLayer(Layer.OneHanded);
        }
        else if (m.FindItemOnLayer(Layer.FirstValid) != null && m.FindItemOnLayer(Layer.FirstValid) is FishingPole)
        {
    ...
    private static MutateEntry[] m_MutateTable = new MutateEntry[]
    {
        ...
        new MutateEntry(0.0, 200.0, -200.0, false, new Type[1] {
                null
            })
    };
                    ...
                    switch (nWave)
                    {
                        ...
                        case 5:                                 // Hats
                        {
                            if (nChance > Utility.Random(100))
                            {
                                ...
                                if (Utility.RandomBool())
                                {
                                    preLoot.Name = sAdj + "skullcap"; preLoot.ItemID = 5444;
                                }
                                else
                                {
                                    preLoot.Name = sAdj + "pirate hat"; preLoot.ItemID = 5915;
                                }
