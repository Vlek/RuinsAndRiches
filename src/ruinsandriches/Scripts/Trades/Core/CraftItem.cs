...
public class CraftItem
{
    ...
    public int EnchantItem(Item item, Mobile from, Type typeRes, CraftSystem craftSystem)
    {
        ...
        {
            skill = (int)(from.Skills[SkillName.Tinkering].Value);          if (pm.NpcGuild == NpcGuild.TinkersGuild)
            {
                guild = 100;
            }
        }
        ...
        {
            skill = (int)(from.Skills[SkillName.Tailoring].Value);          if (pm.NpcGuild == NpcGuild.TailorsGuild)
            {
                guild = 100;
            }
        }
        ...
        {
            skill = (int)(from.Skills[SkillName.Bowcraft].Value);           if (pm.NpcGuild == NpcGuild.ArchersGuild)
            {
                guild = 100;
            }
        }
        ...
        {
            skill = (int)(from.Skills[SkillName.Blacksmith].Value);         if (pm.NpcGuild == NpcGuild.BlacksmithsGuild)
            {
                guild = 100;
            }
        }
        ...
        if (CheckSkills(from, typeRes, craftSystem, ref ignored, ref allRequiredSkills))
        {
            ...
            if (item != null)
            {
                    ...
                    || item is SkinUnicornHelm)
                {
                    ...
                    {
                        BaseArmor woody = (BaseArmor)item; woody.Resource = CraftResource.RegularLeather;
                    }
                ...
                if (item is BaseArmor || item is BaseWeapon)                           // ELVEN WOOD
                {
                    ...
                    if (thisResource == CraftResource.ElvenTree)
                    {
                        ...
                        {
                            ((BaseWeapon)item).Attributes.SpellChanneling = 1; ((BaseWeapon)item).Attributes.NightSight = 1; BaseRunicTool.ApplyAttributesTo((BaseWeapon)item, false, 0, 1, 5, 10);
                        }
                        else if (item is BaseShield)
                        {
                            ((BaseShield)item).Attributes.SpellChanneling = 1; ((BaseShield)item).Attributes.NightSight = 1; BaseRunicTool.ApplyAttributesTo((BaseShield)item, false, 0, 1, 5, 10);
                        }
                        else if (item is BaseArmor)
                        {
                            ((BaseArmor)item).ArmorAttributes.MageArmor = 1; ((BaseArmor)item).Attributes.NightSight = 1; BaseRunicTool.ApplyAttributesTo((BaseArmor)item, false, 0, 1, 5, 10);
                        }
                    }
                    else if (thisResource == CraftResource.Dwarven)
                    {
                        if (item is BaseWeapon)
                        {
                            ((BaseWeapon)item).Attributes.RegenHits = 5; ((BaseWeapon)item).Attributes.AttackChance = 50; BaseRunicTool.ApplyAttributesTo((BaseWeapon)item, false, 0, 1, 5, 10);
                        }
                        else if (item is BaseShield)
                        {
                            ((BaseShield)item).Attributes.RegenHits = 5; ((BaseShield)item).Attributes.DefendChance = 10; BaseRunicTool.ApplyAttributesTo((BaseShield)item, false, 0, 1, 5, 10);
                        }
                        else if (item is BaseArmor)
                        {
                            ((BaseArmor)item).Attributes.RegenHits = 5; ((BaseArmor)item).Attributes.DefendChance = 10; BaseRunicTool.ApplyAttributesTo((BaseArmor)item, false, 0, 1, 5, 10);
                        }
                    }
                }
