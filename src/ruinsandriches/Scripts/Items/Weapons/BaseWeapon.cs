    ...
    public abstract class BaseWeapon : Item, IWeapon, IFactionItem, ICraftable, ISlayer, IDurability
    {
        ...
        public static void CheckMetalColor(BaseWeapon weapon)
        {
                ...
                || weapon is LevelPitchfork)
            {
                ...
                if (weapon.Hue == 0)
                {
                    ...
                    {
                        weapon.Hue = Server.Misc.MaterialInfo.GetMaterialColor("dull copper", "", 0);
                    }
                    else if (weapon.Resource == CraftResource.ShadowIron)
                    {
                        weapon.Hue = Server.Misc.MaterialInfo.GetMaterialColor("shadow iron", "", 0);
                    }
        ...
        public virtual SkillName GetUsedSkill(Mobile m, bool checkSkillAttrs)
        {
            ...
            if (checkSkillAttrs && m_AosWeaponAttributes.UseBestSkill != 0)
            {
                ...
                if (fenc > val)
                {
                    sk = SkillName.Fencing; val = fenc;
                }
                if (mcng > val)
                {
                    sk = SkillName.Bludgeoning; val = mcng;
                }
            }
        ...
        public virtual void OnMiss(Mobile attacker, Mobile defender)
        {
            ...
            if (attacker is BaseCreature)
            {
                ...
                if (bc.AI == AIType.AI_Archer)
                {
                    ...
                    {
                        sound = ((BaseMeleeWeapon)(bc.FindItemOnLayer(Layer.OneHanded))).DefMissSound;
                    }
                    else if (bc.FindItemOnLayer(Layer.TwoHanded) is BaseMeleeWeapon)
                    {
                        sound = ((BaseMeleeWeapon)(bc.FindItemOnLayer(Layer.TwoHanded))).DefMissSound;
                    }
        ...
        public override void GetProperties(ObjectPropertyList list)
        {
            ...
            GenericSellInfo gsi = new GenericSellInfo();
            int itemPrice = gsi.GetSellPriceFor((Item)this, 0);

            if (itemPrice != 0)
            {
                list.Add("Value: " + itemPrice.ToString());
            }
