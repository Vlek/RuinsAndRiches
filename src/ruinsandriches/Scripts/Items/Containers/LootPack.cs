...
public class LootPack
{
                ...
                if (!item.Stackable || !cont.TryDropItem(from, item, false))
                {
                    if (MyServerSettings.GetUnidentifiedChance() >= Utility.RandomMinMax(1, 100))
                    {
                        ...
                        if (item is BaseJewel)
                        {
                            ...
                            if (item is MagicBelt || item is MagicSash || item is MagicRobe || item is MagicHat || item is MagicCloak || item is MagicBoots)
                            {
                        ...
                        else if (item is BaseWeapon || item is BaseArmor)
                        {
                            ...
                            else if (item is BaseWizardStaff)
                            {
                                ...
                                {
                                    item.ItemID = Utility.RandomList(0x0DF2, 0x0DF3, 0x0DF4, 0x0DF5, 0x269D, 0x269E, 0x26BC, 0x26C6, 0x639D, 0x639E, 0x639F, 0x63A0); cont.ItemID = item.ItemID;
                                }
                                else
                                {
                                    item.ItemID = Utility.RandomList(0xE81, 0x13F8, 0xDF1, 0x2D25, 0xE89, 0x0908); cont.ItemID = item.ItemID;
                                }
                            }
                            else if (item is BaseWhip || item is GiftThrowingGloves || item is GiftPugilistGloves || item is LevelThrowingGloves || item is LevelPugilistGloves || item is ThrowingGloves || item is PugilistGloves || item is PugilistGlove)
                            {
                                itemID.VendorCanID = "Leatherworker";
                            }
    ...
    public static LootPack Poor {
        get { return Core.SE ? SePoor : Core.AOS?AosPoor : OldPoor; }
    }
    public static LootPack Meager {
        get { return Core.SE ? SeMeager : Core.AOS?AosMeager : OldMeager; }
    }
    public static LootPack Average {
        get { return Core.SE ? SeAverage : Core.AOS?AosAverage : OldAverage; }
    }
    public static LootPack Rich {
        get { return Core.SE ? SeRich : Core.AOS?AosRich : OldRich; }
    }
    public static LootPack FilthyRich {
        get { return Core.SE ? SeFilthyRich : Core.AOS?AosFilthyRich : OldFilthyRich; }
    }
    public static LootPack UltraRich {
        get { return Core.SE ? SeUltraRich : Core.AOS?AosUltraRich : OldUltraRich; }
    }
    public static LootPack SuperBoss {
        get { return Core.SE ? SeSuperBoss : Core.AOS?AosSuperBoss : OldSuperBoss; }
    }
    #endregion
...
public class LootPackEntry
{
    ...
    public static string MagicItemName(Item item, Mobile m, Region from)
    {
        ...
        {
            name = "lute"; if (Utility.RandomMinMax(1, 2) == 1)
            {
                name = "mandolin";
            }
        }
        ...
        else if (FirstLast == 0)                   // FIRST COMES ADJECTIVE
        {
            ...
            switch (Utility.RandomMinMax(0, val))
            {
                ...
                case 6: if (RegionName != "")
                    {
                        name = sAdjective + " " + name + " from " + RegionName;
                    }
                    HaveNewName = 1; break;
                case 7: if (RegionName != "")
                    {
        ...
        else if (FirstLast == 2)                   // IN CASE IT HAS SPECIAL MATERIAL
        {
            ...
            switch (Utility.RandomMinMax(0, val))
            {
                ...
                case 1: name = name + " of " + xName;                                                                                                                   HaveNewName = 1; break;
                case 2: if (RegionName != "")
                    {
                        name = name + " from " + RegionName;
                    }
                    break;
                case 3: name = name + " of " + eAdjective;                                                                                                              HaveNewName = 1; break;
                case 4: if (RegionName != "")
                    {
                        name = sAdjective + " " + name + " from " + RegionName;
                    }
                    HaveNewName = 1; break;
                case 5: if (RegionName != "")
                    {
                        name = name + " from " + RegionName;
                    }
                    break;
