...
class ContainerFunctions
{
    public static int LockTheContainer(int level, LockableContainer box, int nContainerLockable)
    {
        ...
        {
            box.Locked = false; box.LockLevel = 0; box.MaxLockLevel = 0; box.RequiredSkill = 0;
        }
    ...
    public static void FillTheContainer(int level, LockableContainer box, Mobile opener)
    {
        ...
        int filler = (int)(level / 2) + 1; if (filler < 1)
        {
            filler = 1;
        }
        int fillup = filler + 2;
        ...
        if (box is TreasureMapChest || box is ParagonChest || box is SunkenChest || box is GraveChest)
        {
            IgnoreThisFiller = 1;
        }
        else if (Server.Misc.Worlds.IsOnSpaceship(box.Location, box.Map) || IsSpaceCrate(box.ItemID))
        {
            IgnoreThisFiller = 2;
        }
        ...
        for (int i = 0; i < (filled); ++i)
        {
            ...
            if (iRich > Utility.Random(100) && IgnoreThisFiller == 0)
            {
                ...
                if (idropped is BaseWoodBoard && Utility.RandomMinMax(1, 3) > 1 && Worlds.IsExploringSeaAreas(opener) == true)
                {
                    idropped = new DriftwoodBoard();
                }
                else if (idropped is BaseIngot && Utility.RandomMinMax(1, 3) > 1 && Worlds.IsExploringSeaAreas(opener) == true)
                {
                    idropped = new NepturiteIngot();
                }
                else if (idropped is BaseIngot && Utility.RandomMinMax(1, 3) > 1 && Worlds.GetMyWorld(opener.Map, opener.Location, opener.X, opener.Y) == "the Serpent Island")
                {
                    idropped = new ObsidianIngot();
                }
                else if (idropped is BaseLeather && Utility.RandomMinMax(1, 3) > 1 && Worlds.IsExploringSeaAreas(opener) == true)
                {
                    idropped = new SpinedLeather();
                }
                else if (idropped is BaseIngot && Utility.RandomMinMax(1, 10) == 1 && Worlds.GetMyWorld(opener.Map, opener.Location, opener.X, opener.Y) == "the Savaged Empire")
                {
                    idropped = new SteelIngot();
                }
                else if (idropped is BaseLeather && Utility.RandomMinMax(1, 10) == 1 && Worlds.GetMyWorld(opener.Map, opener.Location, opener.X, opener.Y) == "the Savaged Empire")
                {
                    idropped = new DinosaurLeather();
                }
                else if (idropped is BaseIngot && Utility.RandomMinMax(1, 8) == 1 && Worlds.GetMyWorld(opener.Map, opener.Location, opener.X, opener.Y) == "the Island of Umber Veil")
                {
                    idropped = new BrassIngot();
                }
                else if (idropped is BaseIngot && Utility.RandomMinMax(1, 8) == 1 && Worlds.GetMyWorld(opener.Map, opener.Location, opener.X, opener.Y) == "the Underworld" && opener.Map == Map.SavagedEmpire)
                {
                    idropped = new XormiteIngot();
                }
                else if (idropped is BaseLeather && Utility.RandomMinMax(1, 8) == 1 && Worlds.GetMyWorld(opener.Map, opener.Location, opener.X, opener.Y) == "the Underworld" && opener.Map == Map.SavagedEmpire)
                {
                    idropped = new AlienLeather();
                }
                else if (idropped is BaseIngot && Utility.RandomMinMax(1, 8) == 1 && Worlds.GetMyWorld(opener.Map, opener.Location, opener.X, opener.Y) == "the Underworld")
                {
                ...
                else if (idropped is BaseWoodBoard && Utility.RandomMinMax(1, 10) == 1 && Worlds.GetMyWorld(opener.Map, opener.Location, opener.X, opener.Y) == "the Underworld")
                {
                    idropped = new PetrifiedBoard();
                }
                else if (idropped is BaseWoodBoard && Utility.RandomMinMax(1, 3) > 1 && (reg.IsPartOf(typeof(NecromancerRegion)) || reg.IsPartOf("the Crypts of Dracula") || reg.IsPartOf("the Castle of Dracula")))
                {
            ...
            if (iRich > Utility.Random(500) && IgnoreThisFiller == 0)
            {
                ...
                {
                    Item idropped = new ScrollClue(); box.DropItem(idropped);
                }
                else
                {
                    Item idropped = DungeonLoot.RandomLoreBooks(); box.DropItem(idropped);
                }
            }
            if (iRich > Utility.Random(5000) && IgnoreThisFiller == 0 && opener != null && box is DungeonChest)
            {
                ...
                {
                    Item idropped = new FrankenJournalInBox(); box.DropItem(idropped);
                }
            }
            ...
            {
                int tMapLevel = (int)(level / 2); if (tMapLevel < 1)
                {
                    tMapLevel = 1;
                }
                else if (tMapLevel > 6)
            ...
            if (iRich > Utility.Random(100) && IgnoreThisFiller == 0)
            {
                ...
                {
                    idropped.Name = "Book"; idropped.Hue = Utility.RandomColor(0); idropped.ItemID = RandomThings.GetRandomBookItemID();
                }
            ...
            if (iRich > Utility.Random(1000) && IgnoreThisFiller != 2)
            {
                ...
                if (idropped.Stackable && !(idropped is PotionOfMight) && !(idropped is PotionOfWisdom) && !(idropped is PotionOfDexterity))
                {
                    idropped.Amount = Utility.RandomMinMax(1, 10);
                }
                else if (idropped is Runebook)
                {
                    idropped.ItemID = Utility.RandomList(0x22C5, 0x0F3D, 0x4F50, 0x4F51, 0x5463, 0x5464);
                }
                else if (idropped is CandleLarge || idropped is Candelabra || idropped is CandelabraStand)
                {
                ...
                else if ((idropped is SmallHollowBook || idropped is LargeHollowBook) && Utility.RandomBool())
                {
                    ...
                    {
                        int buk = 1; if (IgnoreThisFiller != 2)
                        {
                            buk = 0;
                        }
                        ...
                        if ((level * 9) > Utility.Random(100))
                        {
                            if (booking == 0)
                            {
                                ...
                                {
                                    int mgsc = rich + 3; if (mgsc > 63)
                                    {
                                        mgsc = Utility.RandomMinMax(56, 63);
                                    }
                                    int elsc = (rich + 3 / 2); if (elsc > 31)
                                    {
                                    ...
                                    if (scroll > 2)
                                    {
                                        Item item = Loot.RandomScroll(0, mgsc, SpellbookType.Regular); iBook.DropItem(item);
                                    }
                                    else if (scroll == 2)
                                    {
                                        Item item = Loot.RandomScroll(0, elsc, SpellbookType.Elementalism); iBook.DropItem(item);
                                    }
                                    else
                                    {
                                        Item item = Loot.RandomScroll(0, 17, SpellbookType.Necromancer); iBook.DropItem(item);
                                    }
                                }
                            }
                            else if (booking == 1)
        ...
        {
            Item slay = DungeonLoot.RandomSlayer(); box.DropItem(slay);
        }
        ...
        if (Utility.RandomMinMax(0, 100) < (level * 5) && CorpseNoFiller == 0 && IgnoreThisFiller != 2)
        {
            ...
            else
            {
                ...
                if (relic is DDRelicWeapon && Server.Misc.GetPlayerInfo.OrientalPlay(opener) == true)
                {
                    Server.Items.DDRelicWeapon.MakeOriental(relic);
                }
                else if (relic is DDRelicStatue && Server.Misc.GetPlayerInfo.OrientalPlay(opener) == true)
                {
                    Server.Items.DDRelicStatue.MakeOriental(relic);
                }
                else if (relic is DDRelicBanner && relic.ItemID != 0x2886 && relic.ItemID != 0x2887 && Server.Misc.GetPlayerInfo.OrientalPlay(opener) == true)
                {
        ...
        if (iRich > Utility.Random(100) && (IgnoreThisFiller == 0 || IgnoreThisFiller == 2))
        {
            ...
            if (MyServerSettings.GetUnidentifiedChance() >= Utility.RandomMinMax(1, 100))
            {
                ...
                if (weapon is BaseWizardStaff)
                {
                    ...
                    {
                        weapon.ItemID = Utility.RandomList(0x0DF2, 0x0DF3, 0x0DF4, 0x0DF5, 0x269D, 0x269E, 0x26BC, 0x26C6, 0x639D, 0x639E, 0x639F, 0x63A0); itemID.ItemID = weapon.ItemID;
                    }
                    else
                    {
                        weapon.ItemID = Utility.RandomList(0xE81, 0x13F8, 0xDF1, 0x2D25, 0xE89, 0x0908); itemID.ItemID = weapon.ItemID;
                    }
                }
                else if (weapon is GiftThrowingGloves || weapon is GiftPugilistGloves || weapon is LevelThrowingGloves || weapon is LevelPugilistGloves || weapon is ThrowingGloves || weapon is PugilistGloves || weapon is PugilistGlove || weapon is BaseWhip)
                {
        ...
        for (int i = 0; i < (filled); ++i)
        {
            ...
            else if (Server.Misc.Worlds.IsOnSpaceship(box.Location, box.Map))
            {
                ...
                if (item is KilrathiHeavyGun || item is KilrathiGun || item is LightSword || item is DoubleLaserSword)
                {
                    ...
                    {
                        item = Server.Misc.ContainerFunctions.LootMutate(from, Server.LootPack.GetRegularLuckChance(from), item, box, level);
                    }
                }
    ...
    public static int BuildContainer(LockableContainer box, int thisHue, int thisItem, int thisGump, int thisDesign)
    {
        ...
        if (myBox == 1)
        {
            ...
            if (thisDesign == 3)
            {
                box.ItemID = Utility.RandomList(0x10EC, 0x10ED);      box.GumpID = 0x976;             box.Name = "Rusty Metal Crate";         box.Hue = 0;
            }
        }
        else if (myBox == 2)
        {
            ...
            if (thisDesign == 3)
            {
                box.ItemID = Utility.RandomList(0x10EA, 0x10EB);      box.GumpID = 0x976;             box.Name = "Metal Crate";               box.Hue = 0;
            }
        }
        else if (myBox == 3)
        {
        ...
        else if (myBox == 5)
        {
            ...
            box.Hue = MaterialInfo.GetMaterialColor(hue, "", box.Hue);
            if (box.ItemID == 0x4C2B || box.ItemID == 0x4C2C || box.ItemID == 0x1C0E || box.ItemID == 0x1C0F)
            {
                box.GumpID = 0x49;
            }
        }
        ...
        else if (myBox == 7)
        {
            ...
            else if (box.ItemID == 0x1248 || box.ItemID == 0x1264 || box.ItemID == 0x5777 || box.ItemID == 0x5776)
            {
                packName = "Sack"; packAdj = "Large ";
            }
        ...
        else if (myBox == 8)
        {
            ...
            {
                packName = "Satchel"; packAdj = "Large ";
            }
            else if (box.ItemID == 0x1C10 || box.ItemID == 0x1CC6)
            {
                packName = "Backpack"; packAdj = "Large ";
            }
        ...
        if (thisDesign == 1)
        {
            ...
            {
                box.Name = box.Name + " Urn"; if (Utility.RandomBool())
                {
            ...
            else if (myBox == 14)
            {
                ...
                if (box.ItemID == 0x27E0 || box.ItemID == 0x280A || box.ItemID == 0x2802 || box.ItemID == 0x2803)
                {
    ...
    public static void RelicValueIncrease(int level, Item item)
    {
        ...
        {
            DDRelicAlchemy relic = (DDRelicAlchemy)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicArmor)
        {
            DDRelicArmor relic = (DDRelicArmor)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicArts)
        {
            DDRelicArts relic = (DDRelicArts)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicBearRugsAddon)
        {
            DDRelicBearRugsAddon relic = (DDRelicBearRugsAddon)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicBook)
        {
            DDRelicBook relic = (DDRelicBook)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicTablet)
        {
            DDRelicTablet relic = (DDRelicTablet)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicClock1)
        {
            DDRelicClock1 relic = (DDRelicClock1)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicClock2)
        {
            DDRelicClock2 relic = (DDRelicClock2)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicClock3)
        {
            DDRelicClock3 relic = (DDRelicClock3)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicCloth)
        {
            DDRelicCloth relic = (DDRelicCloth)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicDrink)
        {
            DDRelicDrink relic = (DDRelicDrink)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicFur)
        {
            DDRelicFur relic = (DDRelicFur)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicInstrument)
        {
            DDRelicInstrument relic = (DDRelicInstrument)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicJewels)
        {
            DDRelicJewels relic = (DDRelicJewels)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicLeather)
        {
            DDRelicLeather relic = (DDRelicLeather)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicLight1)
        {
            DDRelicLight1 relic = (DDRelicLight1)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicLight2)
        {
            DDRelicLight2 relic = (DDRelicLight2)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicLight3)
        {
            DDRelicLight3 relic = (DDRelicLight3)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicOrbs)
        {
            DDRelicOrbs relic = (DDRelicOrbs)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicPainting)
        {
            DDRelicPainting relic = (DDRelicPainting)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicReagent)
        {
            DDRelicReagent relic = (DDRelicReagent)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicRugAddonDeed)
        {
            DDRelicRugAddonDeed relic = (DDRelicRugAddonDeed)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicScrolls)
        {
            DDRelicScrolls relic = (DDRelicScrolls)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicStatue)
        {
            DDRelicStatue relic = (DDRelicStatue)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicVase)
        {
            DDRelicVase relic = (DDRelicVase)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicWeapon)
        {
            DDRelicWeapon relic = (DDRelicWeapon)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicGem)
        {
            DDRelicGem relic = (DDRelicGem)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
        else if (item is DDRelicGrave)
        {
            DDRelicGrave relic = (DDRelicGrave)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue;
        }
    }
        ...
        if (item != null && canMod)
        {
            ...
            if (!plain && MyServerSettings.GetUnidentifiedChance() >= Utility.RandomMinMax(1, 100))
            {
                ...
                if (item is BaseJewel)
                {
                    ...
                    if (item is MagicBelt || item is MagicRobe || item is MagicHat || item is MagicCloak || item is MagicBoots || item is MagicSash)
                    {
                ...
                {
                    itemID.SkillRequired = "ItemID"; itemID.VendorCanID = "Tailor";
                }
                else if (item is BaseQuiver)
                {
                    itemID.SkillRequired = "ItemID"; itemID.VendorCanID = "Bowyer";
                }
                else if (item is BaseInstrument)
                {
                    itemID.SkillRequired = "ItemID"; itemID.VendorCanID = "Bard";
                }
                else if (item is BaseClothing)
                {
                    itemID.SkillRequired = "ItemID"; itemID.VendorCanID = "Tailor"; if (item is BaseShoes)
                    {
                        itemID.VendorCanID = "Leatherworker";
                    }
                }
                else
                {
                    ...
                    else if (item is BaseWhip || item is ThrowingGloves || item is PugilistGloves || item is PugilistGlove)
                    {
                        itemID.VendorCanID = "Leatherworker";
                    }
    ...
    public static void GiveRandomItem(Mobile from)
    {
        ...
        else if (found == 2)
        {
            ...
            if (loot is BaseWoodBoard && Utility.RandomMinMax(1, 3) > 1 && Worlds.IsExploringSeaAreas(from) == true)
            {
                loot = new DriftwoodBoard();
            }
            else if (loot is BaseIngot && Utility.RandomMinMax(1, 3) > 1 && Worlds.IsExploringSeaAreas(from) == true)
            {
                loot = new NepturiteIngot();
            }
            else if (loot is BaseIngot && Utility.RandomMinMax(1, 3) > 1 && Worlds.GetMyWorld(from.Map, from.Location, from.X, from.Y) == "the Serpent Island")
            {
                loot = new ObsidianIngot();
            }
            else if (loot is BaseLeather && Utility.RandomMinMax(1, 3) > 1 && Worlds.IsExploringSeaAreas(from) == true)
            {
                loot = new SpinedLeather();
            }
            else if (loot is BaseIngot && Utility.RandomMinMax(1, 10) == 1 && Worlds.GetMyWorld(from.Map, from.Location, from.X, from.Y) == "the Savaged Empire")
            {
                loot = new SteelIngot();
            }
            else if (loot is BaseLeather && Utility.RandomMinMax(1, 10) == 1 && Worlds.GetMyWorld(from.Map, from.Location, from.X, from.Y) == "the Savaged Empire")
            {
                loot = new DinosaurLeather();
            }
            else if (loot is BaseIngot && Utility.RandomMinMax(1, 8) == 1 && Worlds.GetMyWorld(from.Map, from.Location, from.X, from.Y) == "the Island of Umber Veil")
            {
                loot = new BrassIngot();
            }
            else if (loot is BaseIngot && Utility.RandomMinMax(1, 8) == 1 && Worlds.GetMyWorld(from.Map, from.Location, from.X, from.Y) == "the Underworld" && from.Map == Map.SavagedEmpire)
            {
                loot = new XormiteIngot();
            }
            else if (loot is BaseLeather && Utility.RandomMinMax(1, 8) == 1 && Worlds.GetMyWorld(from.Map, from.Location, from.X, from.Y) == "the Underworld" && from.Map == Map.SavagedEmpire)
            {
                loot = new AlienLeather();
            }
            else if (loot is BaseIngot && Utility.RandomMinMax(1, 8) == 1 && Worlds.GetMyWorld(from.Map, from.Location, from.X, from.Y) == "the Underworld")
            {
            ...
            else if (loot is BaseWoodBoard && Utility.RandomMinMax(1, 10) == 1 && Worlds.GetMyWorld(from.Map, from.Location, from.X, from.Y) == "the Underworld")
            {
                loot = new PetrifiedBoard();
            }
            else if (loot is BaseWoodBoard && Utility.RandomMinMax(1, 3) > 1 && (reg.IsPartOf(typeof(NecromancerRegion)) || reg.IsPartOf("the Crypts of Dracula") || reg.IsPartOf("the Castle of Dracula")))
            {
                loot = new GhostBoard();
            }
        ...
        else if (found == 4)
        {
            ...
            if (loot is BlueBook)
            {
                loot.Name = "Book"; loot.Hue = Utility.RandomColor(0); loot.ItemID = RandomThings.GetRandomBookItemID();
            }
            if ((loot is MetalBox))
            {
        ...
        else if (found == 7)
        {
            ...
            if (odd == 1)
            {
                ...
                if (loot is DDRelicWeapon && Server.Misc.GetPlayerInfo.OrientalPlay(from) == true)
                {
                    Server.Items.DDRelicWeapon.MakeOriental(loot);
                }
                else if (loot is DDRelicStatue && Server.Misc.GetPlayerInfo.OrientalPlay(from) == true)
                {
                    Server.Items.DDRelicStatue.MakeOriental(loot);
                }
                else if (loot is DDRelicBanner && loot.ItemID != 0x2886 && loot.ItemID != 0x2887 && Server.Misc.GetPlayerInfo.OrientalPlay(from) == true)
                {
        ...
        else if (found == 8)
        {
            ...
            {
                loot = Loot.RandomSecretReagent(); UnknownReagent myreg = (UnknownReagent)loot; myreg.RegAmount = Utility.RandomMinMax((1 * 5), (2 * 10)) + 1;
            }
        }
        else if (found == 9)
        {
