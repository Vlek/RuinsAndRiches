...
public abstract class BaseBoat : BaseMulti
{
    ...
    public Hold Hold {
        get { return m_Hold; } set { m_Hold = value; }
    }

    [CommandProperty(AccessLevel.GameMaster)]
    public BoatDoor BoatDoor {
        get { return m_BoatDoor; } set { m_BoatDoor = value; }
    }

    [CommandProperty(AccessLevel.GameMaster)]
    public TillerMan TillerMan {
        get { return m_TillerMan; } set { m_TillerMan = value; }
    }

    [CommandProperty(AccessLevel.GameMaster)]
    public Plank PPlank {
        get { return m_PPlank; } set { m_PPlank = value; }
    }

    [CommandProperty(AccessLevel.GameMaster)]
    public Plank SPlank {
        get { return m_SPlank; } set { m_SPlank = value; }
    }
    ...
    public Direction Facing {
        get { return m_Facing; } set { SetFacing(value); }
    }

    [CommandProperty(AccessLevel.GameMaster)]
    public Direction Moving {
        get { return m_Moving; } set { m_Moving = value; }
    }
    ...
    public bool Anchored {
        get { return m_Anchored; } set { m_Anchored = value; }
    }

    [CommandProperty(AccessLevel.GameMaster)]
    public string ShipName {
        get { return m_ShipName; } set { m_ShipName = value; if (m_TillerMan != null)
                                         {
    ...
    public MapItem MapItem {
        get { return m_MapItem; } set { m_MapItem = value; }
    }

    [CommandProperty(AccessLevel.GameMaster)]
    public int NextNavPoint {
        get { return m_NextNavPoint; } set { m_NextNavPoint = value; }
    }

    [CommandProperty(AccessLevel.GameMaster)]
    public DateTime TimeOfDecay {
        get { return m_DecayTime; } set { m_DecayTime = value; if (m_TillerMan != null)
                                          {
                                              m_TillerMan.InvalidateProperties();
                                          }
        }
    ...
    public static SunkenShip CreateSunkenShip(Mobile from, Mobile killer)
    {
        ...
        level = (int)(level / 3); if (level < 1 && Utility.RandomBool())
        {
            level = 1;
        }
        ...
        {
            ShipWreck.Name = "sunken boat"; if (cycle > 1)
            {
                cycle = 1;
            }
        }
        ...
        {
            Item i1 = new RawFishSteak(Utility.RandomMinMax(1, 8));   ShipWreck.DropItem(i1);
        }
        if (Utility.RandomMinMax(1, 4) == 1)
        {
            Item i2 = new Fish(Utility.RandomMinMax(1, 8));                   ShipWreck.DropItem(i2);
        }
        if (Utility.RandomMinMax(1, 4) == 1)
        {
            Item i3 = new FishingPole();                                                                  ShipWreck.DropItem(i3);
        }
        if (Utility.RandomMinMax(1, 4) == 1)
        {
            Item i4 = new NewFish();                                                                              ShipWreck.DropItem(i4);
        }
        if (Utility.RandomMinMax(1, 4) == 1)
        {
            Item i5 = new Sextant();                                                                              ShipWreck.DropItem(i5);
        }
    ...
    public bool SetFacing(Direction facing)
    {
        ...
        if (Map != Map.Internal)
        {
            switch (facing)
            {
                case Direction.North: if (!CanFit(Location, Map, NorthID))
                    {
                        return false;
                    }
                    break;
                case Direction.East:  if (!CanFit(Location, Map, EastID))
                    {
                        return false;
                    }
                    break;
                case Direction.South: if (!CanFit(Location, Map, SouthID))
                    {
                        return false;
                    }
                    break;
                case Direction.West:  if (!CanFit(Location, Map, WestID))
                    {
                        return false;
                    }
                    break;
