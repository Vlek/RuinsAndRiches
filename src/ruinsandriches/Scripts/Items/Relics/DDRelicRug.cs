...
public class DDRelicRugAddon : BaseAddon
{
    ...
    public int Relic_Color {
        get { return RelicColor; } set { RelicColor = value; InvalidateProperties(); }
    }
    ...
    public int Relic_RugID {
        get { return RelicRugID; } set { RelicRugID = value; InvalidateProperties(); }
    }

    [CommandProperty(AccessLevel.Owner)]
    public int Relic_Found {
        get { return RelicFound; } set { RelicFound = value; InvalidateProperties(); }
    }

    [CommandProperty(AccessLevel.Owner)]
    public string Relic_Quality {
        get { return RelicQuality; } set { RelicQuality = value; InvalidateProperties(); }
    }
...
public class DDRelicRugAddonDeed : BaseAddonDeed
{
    ...
    public DDRelicRugAddonDeed(int RelCost, int RelID, int RelRugID, int RelHue, string RelQuality, int RelRolled)
    {
        ...
        if (Relic_Found > 0)
        {
            sLook = Relic_Quality; RelicQuality = Relic_Quality;
        }
        else if (RelQuality != "blank")
        {
            sLook = RelQuality; RelicQuality = RelQuality;
        }
        else
        {
            sLook = "a rare";
        ...
        if (Relic_Found > 0)
        {
            RelicColor = Relic_Color; Hue = Relic_Color;
        }
        else if (RelRolled > 0)
        {
            RelicColor = RelHue; Hue = RelHue;
        }
        else
        {
        ...
        if (Relic_Found > 0)
        {
            RelicMainID = Relic_MainID; ItemID = Relic_MainID;
        }
        else if (RelID > 0)
        {
            RelicMainID = RelID; ItemID = RelID;
        }
        else
        {
            ItemID = Utility.RandomList(0x3F08, 0x3F09, 0x3F0A, 0x3F0D, 0x3F17, 0x3F18, 0x3F11); RelicMainID = ItemID;
        }
