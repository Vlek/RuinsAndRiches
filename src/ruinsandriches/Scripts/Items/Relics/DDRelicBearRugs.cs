...
public class DDRelicBearRugsAddonDeed : BaseAddonDeed
{
    ...
    public DDRelicBearRugsAddonDeed(int RelCost, int RelID, int RelRugID, int RelHue, string RelQuality, int RelRolled)
    {
        ...
        if (Relic_Found > 0)
        {
            RelicMainID = Relic_MainID; ItemID = Relic_MainID; RelicColor = Relic_Color; Hue = Relic_Color; RelicRugID = Relic_RugID;
        }
        else if (RelID > 0)
        {
            RelicMainID = RelID; ItemID = RelID; RelicColor = RelHue; Hue = RelHue; RelicRugID = RelRugID;
        }
        else
        {
            switch (Utility.RandomMinMax(0, 5))
