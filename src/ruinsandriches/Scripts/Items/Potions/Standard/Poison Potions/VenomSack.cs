    ...
    public override void OnDoubleClick(Mobile from)
    {
        ...
        if (from.CheckSkill(SkillName.Poisoning, nSkill, 125))
        {
            ...
            else
            {
                ...
                if (this.Name == "lesser venom sack")
                {
                    from.AddToBackpack(new LesserPoisonPotion()); sPotion = "lesser poison";
                }
                else if (this.Name == "venom sack")
                {
                    from.AddToBackpack(new PoisonPotion()); sPotion = "poison";
                }
                else if (this.Name == "greater venom sack")
                {
                    from.AddToBackpack(new GreaterPoisonPotion()); sPotion = "greater poison";
                }
                else if (this.Name == "deadly venom sack")
                {
                    from.AddToBackpack(new DeadlyPoisonPotion()); sPotion = "deadly poison";
                }
                else
                {
                    from.AddToBackpack(new LethalPoisonPotion()); sPotion = "lethal poison";
                }
